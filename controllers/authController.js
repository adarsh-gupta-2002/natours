...
//     // res.cookie('testing', 'test_value', cookieOptions);
...
exports.resetPassword = catchAsync(async (req, res, next) => {
    ...
    // 4) log the user in , sent json web token to the client
    // const token = signToken(user._id)
    // res.status(200).json({
    //     status: 'success',
    //     token
...
exports.isLoggedIn =  async (req , res , next) => {
    ...
    try{
        ...
        if(req.cookies.jwt){
            ...
            // 1) verify token 
            // const decoded = await promisify(token.verify)(jwt , process.env.JWT_SECRET )
            const decoded = await verifyToken(req.cookies.jwt, process.env.JWT_SECRET);
    
            // 2) Check if user still sxists
            const currentUser = await User.findById(decoded.id)
            if(!currentUser){
                return next()
            }
